...
import { testarConexaoSlackEntrada } from "@/lib/slack-arquivos";
...
export async function POST() {
  ...
  try {
    const r = await testarConexaoSlackEntrada();
    return NextResponse.json(r);
  } catch (err: any) {
    return NextResponse.json({ ok: false, error: err?.message ?? "Erro ao testar o Slack." }, { status: 500 });
  }
}
