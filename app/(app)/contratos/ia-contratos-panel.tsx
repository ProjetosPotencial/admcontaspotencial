...
const SUGESTOES = [
  ...
  "Tem algum contrato suspenso?",
];

export default function IaContratosPanel() {
  const [mensagens, setMensagens] = useState<Msg[]>([
    { role: "assistant", texto: "Olá! Sou o assistente de contratos. Pergunta sobre vencimentos, valores, lojas ou empresas que eu busco no sistema pra você." },
  ]);
  ...
  async function enviar(pergunta: string) {
    ...
    try {
      const resp = await fetch("/api/ia-contratos", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          mensagens: novasMsgs.map((m) => ({ role: m.role, content: m.texto })),
        }),
      });
      const json = await resp.json();
      if (!resp.ok || json.error) {
        setErro(json.error ?? "Erro ao consultar a IA.");
        setMensagens((m) => m.slice(0, -1)); // desfaz a pergunta que falhou
        setTexto(pergunta);
      } else {
  ...
  return (
    <div className="card flex flex-col h-[560px]">
      <div className="flex items-center gap-2.5 px-4 py-3.5 border-b border-linha2">
        ...
        <div>
          <div className="text-[13.5px] font-bold text-[#1a1a1a]">IA Assistente de Contratos</div>
          <div className="text-[11px] text-[#6c757d]">Online</div>
        </div>
      </div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); enviar(texto); }} className="flex items-center gap-2 px-3 py-3 border-t border-linha2">
        <input
