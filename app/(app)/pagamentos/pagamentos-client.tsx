      ...
      <section>
        ...
        <div className="card divide-y divide-[#f1f3f5]">
          {prontos.map((i) => {
            ...
            return (
              <div key={i.id} className="flex items-center gap-3 px-5 py-3.5 text-[13px] flex-wrap">
                ...
                <select value={formaEscolhida[i.id] ?? ""} onChange={(e) => setFormaEscolhida((f) => ({ ...f, [i.id]: e.target.value }))}
                  className="border border-linha rounded-md px-2 py-1.5 text-[12px]">
                ...
                <button onClick={() => marcarPago(i)} disabled={!formaEscolhida[i.id] || marcando === i.id}
                  className="bg-ok hover:bg-ok-dark text-white rounded-md px-3.5 py-1.5 text-[12px] font-semibold disabled:opacity-40 transition-colors">
                  {marcando === i.id ? "Salvando..." : "Marcar como pago"}
                </button>
              </div>
            );
          })}
