...
export default async function ContasPage() {
  ...
  const [
    ...
    supabase
      .from("contas")
      .select("id, tipo, fornecedor_nome, identificador, dia_vencimento, origem, cnpj_cpf, insc_cod_mat, portal_link, eh_rateio, rateio_divisor, observacoes, status, data_encerramento, motivo_encerramento, chamado_numero, numero_nf, remetente_cnpj, destinatario_razao, destinatario_cnpj, chave_acesso, loja_id, lojas ( codigo, coban, empresas ( nome ) )")
  ...
  // só conta como pendente o que ainda NÃO foi lançado. Assim que o boleto
  // é lançado, a conta sai de "atrasadas"/"vence hoje" - ela já foi processada.
  const pendentes = (lancDetalhado ?? []).filter((l: any) => l.situacao === "pendente");
