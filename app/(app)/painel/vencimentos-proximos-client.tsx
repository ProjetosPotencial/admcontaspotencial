...
type Item = {
  ...
  contas: { id: string; tipo: string; dia_vencimento: number | null; fornecedor_nome: string | null; lojas: { codigo: string } | null };
};
    ...
    <div className="card overflow-hidden">
      ...
      <div>
        {filtrados.slice(0, 8).map((l) => {
          ...
          return (
            <Link key={l.id} href={`/contas?conta=${l.contas.id}`} className="flex items-center gap-3.5 px-5 py-3 border-b border-linha2 last:border-0 hover:bg-[#f8f9fa] transition">
        ...
        {filtrados.length === 0 && (
          <div className="text-center py-10 text-[#adb5bd] text-[13px]">Nada vencendo nessa janela.</div>
        )}
      </div>
    </div>
  );
