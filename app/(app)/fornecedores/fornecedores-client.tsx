        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            {filtrados.map((f) => {
              ...
              return (
                <tr key={f.id} className="h-12 border-b border-[#f0f0f0] last:border-0 hover:bg-[#f9f9f9]">
                  ...
                  <td className="px-4 text-[12px] text-[#999] font-mono truncate max-w-[220px]">{f.portal_padrao ?? "—"}</td>
                </tr>
              );
            })}
            {filtrados.length === 0 && (
              <tr><td colSpan={3} className="text-center py-12 text-[#999]">Nenhum fornecedor encontrado.</td></tr>
            )}
          </tbody>
        </table>
      </div>
