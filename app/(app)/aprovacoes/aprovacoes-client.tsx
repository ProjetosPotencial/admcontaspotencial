...
type Item = {
  ...
  contas: {
    tipo: string; fornecedor_nome: string | null; identificador?: string | null; eh_rateio: boolean;
...
export default function AprovacoesClient({ itens, resumoMes, solicitantes = {} }: {
  ...
  const empresas = useMemo(() => Array.from(new Set(fila.map((i) => i.contas.lojas?.empresas?.nome).filter(Boolean))).sort() as string[], [fila]);
  const lojasLista = useMemo(() => Array.from(new Set(fila.map((i) => i.contas.lojas?.codigo).filter(Boolean))).sort() as string[], [fila]);
  const solicLista = useMemo(() => Array.from(new Set(fila.map((i) => i.lancado_por).filter(Boolean))) as string[], [fila]);
  ...
  const filtrados = useMemo(() => {
    ...
    return fila.filter((i) => {
      ...
      if (fTipo !== "todos" && i.contas.tipo !== fTipo) return false;
      if (fSolic !== "todos" && i.lancado_por !== fSolic) return false;
      const v = Number(i.valor ?? 0);
      if (vMin && v < Number(vMin)) return false;
      if (vMax && v > Number(vMax)) return false;
      if (fDe || fAte) {
        const d = i.lancado_em ? i.lancado_em.slice(0, 10) : "";
        if (fDe && d < fDe) return false;
        if (fAte && d > fAte) return false;
      }
      if (q) {
        // busca inteligente: fornecedor, loja, empresa, cidade, boleto,
        // identificador (nota/matrícula) e observações
        const alvo = [
          i.contas.fornecedor_nome, i.contas.identificador, L?.codigo, L?.cidade, L?.uf,
          L?.empresas?.nome, i.codigo_barras, i.observacao,
          TIPOS[i.contas.tipo]?.n, solicitantes[i.lancado_por ?? ""],
  ...
  async function decidir(item: Item, aprovar: boolean, motivo?: string) {
    ...
    // avisa o Slack (quem decidiu, loja, tipo, valor) sem travar a tela
        ...
        <div className="card p-4 mb-4">
          <div className="flex flex-wrap gap-2 items-center">
            <div className="relative flex-1 min-w-[240px]">
              <svg className="absolute left-3 top-1/2 -translate-y-1/2" width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="#adb5bd" strokeWidth="1.8"><circle cx="9" cy="9" r="6" /><path d="M13.5 13.5L17 17" /></svg>
            ...
            <select value={fLoja} onChange={(e) => setFLoja(e.target.value)} className="h-10 bg-white border border-linha rounded-md px-3 text-[13px]">
              <option value="todas">Todas as lojas</option>
              {lojasLista.map((l) => <option key={l} value={l}>{l}</option>)}
            </select>
            <select value={fTipo} onChange={(e) => setFTipo(e.target.value)} className="h-10 bg-white border border-linha rounded-md px-3 text-[13px]">
              <option value="todos">Todo tipo de despesa</option>
              {Object.entries(TIPOS).map(([k, v]) => <option key={k} value={k}>{v.n}</option>)}
            </select>
            <select value={fSolic} onChange={(e) => setFSolic(e.target.value)} className="h-10 bg-white border border-linha rounded-md px-3 text-[13px]">
              <option value="todos">Qualquer solicitante</option>
              {solicLista.map((id) => <option key={id} value={id}>{solicitantes[id] ?? "—"}</option>)}
            </select>
          </div>
          <div className="flex flex-wrap gap-2 items-center mt-2">
            <label className="flex items-center gap-1.5 text-[12px] text-[#6c757d]">De
              <input type="date" value={fDe} onChange={(e) => setFDe(e.target.value)} className="h-9 bg-white border border-linha rounded-md px-2 text-[12.5px]" /></label>
            <label className="flex items-center gap-1.5 text-[12px] text-[#6c757d]">até
              <input type="date" value={fAte} onChange={(e) => setFAte(e.target.value)} className="h-9 bg-white border border-linha rounded-md px-2 text-[12.5px]" /></label>
            <label className="flex items-center gap-1.5 text-[12px] text-[#6c757d]">Valor de
              <input type="number" value={vMin} onChange={(e) => setVMin(e.target.value)} placeholder="0" className="h-9 w-[92px] bg-white border border-linha rounded-md px-2 text-[12.5px]" /></label>
            <label className="flex items-center gap-1.5 text-[12px] text-[#6c757d]">até
              <input type="number" value={vMax} onChange={(e) => setVMax(e.target.value)} placeholder="—" className="h-9 w-[92px] bg-white border border-linha rounded-md px-2 text-[12.5px]" /></label>
            {temFiltro && (
              <button onClick={limpar} className="flex items-center gap-1.5 text-[12.5px] text-[#6c757d] hover:text-alerr font-medium ml-1">
